...
def candles_to_df(raw: list[list]) -> pd.DataFrame:
    ...
    rows = []
    for r in raw:
        try: rows.append({"time": pd.to_datetime(int(r[0]), unit="ms", utc=True), "open": float(r[1]), "high": float(r[2]), "low": float(r[3]), "close": float(r[4]), "volume": float(r[5])})
...
def enrich(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rs = gain / loss.replace(0, float("nan")); x["rsi"] = (100 - (100 / (1 + rs))).fillna(50.0)
    ...
    x["atr"] = tr.rolling(14).mean(); x["vol_ma20"] = x.volume.rolling(20).mean(); x["range_high"] = x.high.rolling(20).max().shift(1); x["range_low"] = x.low.rolling(20).min().shift(1)
    return x


def structure(df: pd.DataFrame) -> dict:
    if len(df) < 30: return {"trend": "unknown", "bos_up": False, "bos_down": False, "sweep_high": False, "sweep_low": False}
    x = enrich(df); last = x.iloc[-1]; prev = x.iloc[-2]; trend = "bullish" if last.ema20 > last.ema50 else "bearish"
    bos_up = bool(last.close > last.range_high) if pd.notna(last.range_high) else False; bos_down = bool(last.close < last.range_low) if pd.notna(last.range_low) else False
    sweep_high = bool(last.high > prev.range_high and last.close < prev.range_high) if pd.notna(prev.range_high) else False; sweep_low = bool(last.low < prev.range_low and last.close > prev.range_low) if pd.notna(prev.range_low) else False
